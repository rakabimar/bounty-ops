...
import type { ApiEndpointDetailDto, ApiEndpointDto, AssetDetailResponse, AssetDto, AssetInventoryFilters, AuditFilters, AuditLogDto, AuthUser, BulkScopeGuardPreflightInput, ChecklistItemDto, CreateChecklistItemRequest, CreateChecklistRequest, CreateEndpointInput, CreateEvidenceItemRequest, CreateHeaderInput, CreateInterestingRequestRequest, CreateJobRequest, CreateProgramInput, CreateResearchNoteRequest, CreateScannerFindingInput, CreateScopeInput, CreateUrlInput, DnsRecordDto, EndpointFilters, EntityChangeDto, EntityWorkspaceSummaryDto, EvidenceItemDto, HttpServiceDto, InterestingRequestDto, JobDto, JobFilters, JobLogsDto, JobQueueHealthDto, LiveHostFilters, ManualChecklistDto, ProgramDetailDto, ProgramDto, ProgramFilters, ProgramHeaderDto, ProgramRulesDto, ProgramScopeDto, ResearchNoteDto, RetryJobInput, ScannerFindingDetailDto, ScannerFindingDto, ScannerFindingFilters, ScannerFindingStatus as CoreScannerFindingStatus, ScopeGuardPreflightInput, ScopeGuardPreflightResult, ScopeGuardSummary, ScoreExplanationDto, ScoringConfigDto, ScoringPreviewRequest, ScoringPreviewResponse, ToolHealthDto, UpdateChecklistItemRequest, UpdateHeaderInput, UpdateProgramInput, UpdateResearchNoteRequest, UpdateRulesInput, UpdateScopeInput, UrlDetailDto, UrlDto, UrlFilters, WorkspaceEntityType } from "./api-types";
...
const mockScopeGuardSummary=async(programId:string):Promise<ScopeGuardSummary>=>{await wait();const db=readDb();const program=db.programs.find(item=>item.id===programId);if(!program)throw new ApiError(404,"NOT_FOUND","Program not found");const rules=db.rules.find(item=>item.programId===programId);const allowed=program.status==="active"&&program.hunting==="ongoing"?["passive","active_light","active_medium"] as const:[];return {programId,programStatus:program.status,huntingStatus:program.hunting==="ongoing"?"ongoing":"not_hunting",automationAllowed:rules?.automation??"unknown",aggressiveAllowed:rules?.aggressive??false,rateLimitRps:rules?.rateLimit||3,maxConcurrency:rules?.concurrency||2,inScopeCount:db.scopes.filter(item=>item.programId===programId&&item.scope==="in_scope").length,outOfScopeCount:db.scopes.filter(item=>item.programId===programId&&item.scope==="out_of_scope").length,requiredHeadersCount:rules?.headers.filter(item=>item.enabled).length??0,forbiddenActions:rules?.forbidden??[],defaultAllowedStages:[...allowed],defaultBlockedStages:["active_deep","manual_approval","blocked"],warnings:rules?.automation==="unknown"?["automation_permission_unknown"]:[]};};
const mockScopeGuardPreflight=async(input:ScopeGuardPreflightInput):Promise<ScopeGuardPreflightResult>=>{await wait();const db=readDb();const program=db.programs.find(item=>item.id===input.programId);const rules=db.rules.find(item=>item.programId===input.programId);let host=input.target.trim().toLowerCase();let scheme:string|null=null;let path:string|null=null;try{const url=new URL(host);scheme=url.protocol.slice(0,-1);host=url.hostname.toLowerCase();path=url.pathname}catch{host=host.split(":")[0]??host}const scopes=db.scopes.filter(item=>item.programId===input.programId);const matches=(scope:ScopeAsset)=>scope.type==="wildcard"?host!==scope.normalized&&host.endsWith(`.${scope.normalized}`):host===scope.normalized||input.target.toLowerCase().startsWith(scope.normalized.toLowerCase());const inScope=scopes.find(item=>item.scope==="in_scope"&&matches(item));const outScope=scopes.find(item=>item.scope==="out_of_scope"&&matches(item));const stage=input.stage??RECON_JOB_DEFAULT_STAGES[input.jobType];const manual=stage==="manual_approval";const blocked=!program||program.status!=="active"||program.hunting!=="ongoing"||!inScope||Boolean(outScope)||(manual&&!input.manualApproved)||(stage!=="passive"&&rules?.automation==="no");const decision=blocked?"blocked":rules?.automation==="unknown"?"limited":"allowed";const headers=(rules?.headers??[]).map(item=>({name:item.name,value:item.value,isRequired:item.enabled}));return {decision,allowed:decision!=="blocked",programId:input.programId,target:input.target,normalizedTarget:{input:input.target,normalized:input.target.trim().toLowerCase(),host,scheme,port:null,path,type:scheme?"url":"host"},jobType:input.jobType,stage,matchedInScope:Boolean(inScope),matchedInScopeScopeId:inScope?.id??null,matchedOutOfScope:Boolean(outScope),matchedOutOfScopeScopeId:outScope?.id??null,scopeReasons:outScope?["target_out_of_scope"]:inScope?["target_in_scope"]:["no_matching_in_scope_rule"],automationAllowed:rules?.automation??"unknown",manualApprovalRequired:manual,effectiveRateLimitRps:Math.min(rules?.rateLimit||3,decision==="limited"?3:Number.MAX_SAFE_INTEGER),effectiveMaxConcurrency:Math.min(rules?.concurrency||2,decision==="limited"?2:Number.MAX_SAFE_INTEGER),requiredHeaders:headers,effectiveHeaders:headers,allowedStages:["passive","active_light","active_medium"],blockedStages:["active_deep","manual_approval","blocked"],reasons:outScope?["target_out_of_scope"]:blocked?["scope_guard_blocked"]:decision==="limited"?["automation_unknown_conservative_limits"]:["automation_allowed"]};};
const mockJobDto=(job:ReconJob):JobDto=>({id:job.id,programId:job.programId,type:"http_probe",status:job.status as JobDto["status"],stage:"active_light",target:null,config:null,requestedByUserId:"mock-admin",manualApproved:false,scopeGuardDecision:null,error:job.error??null,createdAt:now(),updatedAt:now(),runs:[{id:`run-${job.id}`,jobId:job.id,programId:job.programId,type:"http_probe",status:job.status as JobDto["status"],startedAt:null,finishedAt:null,durationMs:null,error:job.error??null,logs:job.logs.map(message=>({timestamp:now(),level:"info",message})),resultSummary:{simulated:true},createdAt:now(),updatedAt:now(),toolRuns:[]}]});
const mockUrlDto=(item:ReturnType<typeof readDb>["urls"][number]):UrlDto=>{let parsed:URL|null=null;try{parsed=new URL(item.url)}catch{}return {id:item.id,programId:item.programId,assetId:item.assetId,httpServiceId:null,url:item.url,normalizedUrl:item.url,scheme:parsed?.protocol.replace(":","")??null,host:item.host,port:parsed?.port?Number(parsed.port):parsed?.protocol==="https:"?443:80,path:parsed?.pathname??null,queryParamKeys:parsed?[...parsed.searchParams.keys()]:[],title:item.title,statusCode:item.statusCode,contentType:null,contentLength:null,responseTimeMs:null,redirectLocation:null,technologies:[],server:null,categories:item.categories as UrlDto["categories"],reasonTags:item.reasons as UrlDto["reasonTags"],sourceTools:[item.source],scopeStatus:"in_scope",status:"new",autoScore:item.autoScore,manualScore:item.manualScore,finalScore:item.finalScore,priority:priorityFromScore(item.finalScore),confidence:75,firstSeenAt:item.firstSeen,lastSeenAt:item.lastSeen,lastReviewedAt:null,createdAt:item.firstSeen,updatedAt:item.lastSeen};};
const mockEndpointDto=(item:ReturnType<typeof readDb>["discoveredEndpoints"][number]):ApiEndpointDto=>({id:item.id,programId:item.programId,assetId:item.assetId,urlId:item.urlId??null,method:item.method,path:item.path,fullUrl:item.fullUrl,normalizedFullUrl:item.fullUrl,statusCode:item.statusCode??null,contentType:item.contentType??null,authRequired:item.authRequired,source:item.source,status:item.status,categories:item.categories as ApiEndpointDto["categories"],reasonTags:item.reasonTags as ApiEndpointDto["reasonTags"],autoScore:item.autoScore,manualScore:item.manualScore,finalScore:item.finalScore,priority:item.priority,confidence:item.confidence,notesCount:item.notesCount,firstSeenAt:item.firstSeenAt,lastSeenAt:item.lastSeenAt,createdAt:item.firstSeenAt,updatedAt:item.lastSeenAt,parametersCount:item.parameters.length,scannerFindingsCount:readDb().scannerFindings.filter(f=>f.endpointId===item.id).length,parameters:item.parameters.map(p=>({id:p.id,endpointId:item.id,name:p.name,location:p.location,exampleValue:p.exampleValue??null,source:p.source,interesting:p.interesting,frequency:p.frequency??null,createdAt:item.firstSeenAt,updatedAt:item.lastSeenAt}))});
const mockFindingDto=(item:ReturnType<typeof readDb>["scannerFindings"][number]):ScannerFindingDto=>({id:item.id,programId:item.programId,assetId:item.assetId,urlId:item.urlId??null,endpointId:item.endpointId??null,tool:item.tool,severity:item.severity,templateId:item.templateId??null,name:item.name,description:item.description??null,matcher:item.matcher??null,matchedUrl:item.matchedUrl??null,evidenceSnippet:item.evidenceSnippet??null,extractedResults:item.extractedResults??null,status:item.status,firstSeenAt:item.firstSeenAt,lastSeenAt:item.lastSeenAt,createdAt:item.firstSeenAt,updatedAt:item.lastSeenAt});
...
export const coreApi={
  ...
  getScopeGuardSummary:async(programId:string):Promise<ScopeGuardSummary>=>API_MODE==="http"?request(`/programs/${programId}/scope-guard/summary`):mockScopeGuardSummary(programId),
  runScopeGuardPreflight:async(input:ScopeGuardPreflightInput):Promise<ScopeGuardPreflightResult>=>API_MODE==="http"?request("/scope-guard/preflight",{method:"POST",body:JSON.stringify(input)}):mockScopeGuardPreflight(input),
  runBulkScopeGuardPreflight:async(programId:string,input:BulkScopeGuardPreflightInput):Promise<ScopeGuardPreflightResult[]>=>API_MODE==="http"?request(`/programs/${programId}/scope-guard/bulk-preflight`,{method:"POST",body:JSON.stringify(input)}):Promise.all(input.targets.map(target=>mockScopeGuardPreflight({programId,target,jobType:input.jobType,stage:input.stage,manualApproved:input.manualApproved}))),
  getJobs:async(filters:JobFilters={}):Promise<JobDto[]>=>API_MODE==="http"?request(`/jobs${qs({...filters})}`):api.jobs(filters.programId).then(items=>items.map(mockJobDto)),
  createJob:async(input:CreateJobRequest):Promise<JobDto>=>{
    if(API_MODE==="http")return request("/jobs",{method:"POST",body:JSON.stringify(input)});
    const item=await api.runJob(input.type,input.programId);return mockJobDto(item);
  },
  getJob:async(id:string):Promise<JobDto>=>{if(API_MODE==="http")return request(`/jobs/${id}`);const item=(await api.jobs()).find(job=>job.id===id);if(!item)throw new ApiError(404,"NOT_FOUND","Job not found");return mockJobDto(item);},
  getJobLogs:async(id:string):Promise<JobLogsDto>=>{if(API_MODE==="http")return request(`/jobs/${id}/logs`);const item=(await api.jobs()).find(job=>job.id===id);if(!item)throw new ApiError(404,"NOT_FOUND","Job not found");return {jobId:id,jobRunId:`run-${id}`,logs:item.logs.map(message=>({timestamp:now(),level:"info",message}))};},
  ...
  getAsset:async(id:string):Promise<AssetDetailResponse>=>API_MODE==="http"?request(`/assets/${id}`):coreApi.getAssets().then(items=>{const asset=items.find(item=>item.id===id);if(!asset)throw new ApiError(404,"NOT_FOUND","Asset not found");const db=readDb();return {asset,dnsRecords:[],httpServices:[],urlsCount:db.urls.filter(item=>item.assetId===id).length,endpointsCount:db.discoveredEndpoints.filter(item=>item.assetId===id).length,scannerFindingsCount:db.scannerFindings.filter(item=>item.assetId===id).length,changes:[]};}),
  getAssetDetail:async(id:string):Promise<AssetDetailResponse>=>coreApi.getAsset(id),
  ...
  getAssetUrls:async(id:string,filters:Omit<UrlFilters,"assetId">={}):Promise<UrlDto[]>=>API_MODE==="http"?request(`/assets/${id}/urls${qs({...filters})}`):coreApi.getUrls({assetId:id,...filters}),
  getAssetEndpoints:async(id:string,filters:Omit<EndpointFilters,"assetId">={}):Promise<ApiEndpointDto[]>=>API_MODE==="http"?request(`/assets/${id}/endpoints${qs({...filters})}`):coreApi.getEndpoints({assetId:id,...filters}),
  getAssetScannerFindings:async(id:string,filters:Omit<ScannerFindingFilters,"assetId">={}):Promise<ScannerFindingDto[]>=>API_MODE==="http"?request(`/assets/${id}/scanner-findings${qs({...filters})}`):coreApi.getScannerFindings({assetId:id,...filters}),
  ...
  getUrls:async(filters:UrlFilters={}):Promise<UrlDto[]>=>API_MODE==="http"?request(`/urls${qs({...filters})}`):api.urls(filters.programId).then(items=>items.filter(item=>!filters.assetId||item.assetId===filters.assetId).map(mockUrlDto)),
  getUrlDetail:async(id:string):Promise<UrlDetailDto>=>{
    if(API_MODE==="http")return request(`/urls/${id}`);const url=(await coreApi.getUrls()).find(item=>item.id===id);if(!url)throw new ApiError(404,"NOT_FOUND","URL not found");const asset=(await coreApi.getAssets()).find(item=>item.id===url.assetId)??null;const endpoints=await coreApi.getEndpoints({urlId:id});const scannerFindings=await coreApi.getScannerFindings({urlId:id});return {url,asset,httpService:null,endpoints,scannerFindings,parameters:endpoints.flatMap(item=>item.parameters??[]),changes:[],scoreExplanation:await coreApi.getUrlScoreExplanation(id)};
  },
  createUrl:async(input:CreateUrlInput):Promise<UrlDto>=>{
    if(API_MODE==="http")return request("/urls",{method:"POST",body:JSON.stringify(input)});const item=await mutate(db=>{const saved={id:`u${Date.now()}`,assetId:input.assetId??"",programId:input.programId,url:input.url,host:new URL(input.url).hostname,statusCode:input.statusCode??200,title:input.title??"Manual URL",categories:[],reasons:["new_asset"],autoScore:2,manualScore:null,finalScore:2,scoreEvents:[],source:"manual",firstSeen:now(),lastSeen:now()};db.urls.unshift(saved);return saved;});return mockUrlDto(item);
  },
  updateUrlStatus:async(id:string,status:AssetStatus):Promise<UrlDto>=>{if(API_MODE==="http")return request(`/urls/${id}/status`,{method:"PATCH",body:JSON.stringify({status})});const item=await mutate(db=>{const url=db.urls.find(value=>value.id===id);if(url)url.interesting=status==="promising";return url});if(!item)throw new ApiError(404,"NOT_FOUND","URL not found");return mockUrlDto(item);},
  updateUrlManualScore:async(id:string,manualScore:number|null):Promise<UrlDto>=>{if(API_MODE==="http")return request(`/urls/${id}/manual-score`,{method:"PATCH",body:JSON.stringify({manualScore})});const item=await mutate(db=>{const url=db.urls.find(value=>value.id===id);if(url){url.manualScore=manualScore;url.finalScore=manualScore??url.autoScore}return url});if(!item)throw new ApiError(404,"NOT_FOUND","URL not found");return mockUrlDto(item);},
  getUrlScoreExplanation:async(id:string):Promise<ScoreExplanationDto>=>{if(API_MODE==="http")return request(`/urls/${id}/score-explanation`);const item=(await coreApi.getUrls()).find(value=>value.id===id);if(!item)throw new ApiError(404,"NOT_FOUND","URL not found");return {entityType:"url",entityId:id,autoScore:item.autoScore,manualScore:item.manualScore,finalScore:item.finalScore,priority:item.priority,confidence:item.confidence,categories:item.categories??[],reasonTags:item.reasonTags??[],events:[]};},
  getEndpoints:async(filters:EndpointFilters={}):Promise<ApiEndpointDto[]>=>API_MODE==="http"?request(`/endpoints${qs({...filters})}`):api.discoveredEndpoints(filters.programId).then(items=>items.filter(item=>(!filters.assetId||item.assetId===filters.assetId)&&(!filters.urlId||item.urlId===filters.urlId)).map(mockEndpointDto)),
  getEndpointDetail:async(id:string):Promise<ApiEndpointDetailDto>=>{if(API_MODE==="http")return request(`/endpoints/${id}`);const endpoint=(await coreApi.getEndpoints()).find(item=>item.id===id);if(!endpoint)throw new ApiError(404,"NOT_FOUND","Endpoint not found");return {endpoint,asset:(await coreApi.getAssets()).find(item=>item.id===endpoint.assetId)??null,url:endpoint.urlId?(await coreApi.getUrls()).find(item=>item.id===endpoint.urlId)??null:null,parameters:endpoint.parameters??[],scannerFindings:await coreApi.getScannerFindings({endpointId:id}),changes:[],scoreExplanation:await coreApi.getEndpointScoreExplanation(id)};},
  createEndpoint:async(input:CreateEndpointInput):Promise<ApiEndpointDto>=>{if(API_MODE==="http")return request("/endpoints",{method:"POST",body:JSON.stringify(input)});throw new ApiError(501,"NOT_IMPLEMENTED","Mock endpoint creation is not implemented");},
  updateEndpointStatus:async(id:string,status:AssetStatus):Promise<ApiEndpointDto>=>{if(API_MODE==="http")return request(`/endpoints/${id}/status`,{method:"PATCH",body:JSON.stringify({status})});await api.updateEndpointStatus(id,status);const item=(await coreApi.getEndpoints()).find(value=>value.id===id);if(!item)throw new ApiError(404,"NOT_FOUND","Endpoint not found");return item;},
  updateEndpointManualScore:async(id:string,manualScore:number|null):Promise<ApiEndpointDto>=>{if(API_MODE==="http")return request(`/endpoints/${id}/manual-score`,{method:"PATCH",body:JSON.stringify({manualScore})});await api.updateEndpointManualScore(id,manualScore);const item=(await coreApi.getEndpoints()).find(value=>value.id===id);if(!item)throw new ApiError(404,"NOT_FOUND","Endpoint not found");return item;},
  getEndpointScoreExplanation:async(id:string):Promise<ScoreExplanationDto>=>{if(API_MODE==="http")return request(`/endpoints/${id}/score-explanation`);const item=(await coreApi.getEndpoints()).find(value=>value.id===id);if(!item)throw new ApiError(404,"NOT_FOUND","Endpoint not found");return {entityType:"endpoint",entityId:id,autoScore:item.autoScore,manualScore:item.manualScore,finalScore:item.finalScore,priority:item.priority,confidence:item.confidence,categories:item.categories??[],reasonTags:item.reasonTags??[],events:[]};},
  getScannerFindings:async(filters:ScannerFindingFilters={}):Promise<ScannerFindingDto[]>=>API_MODE==="http"?request(`/scanner-findings${qs({...filters})}`):api.scannerFindings(filters.programId).then(items=>items.filter(item=>(!filters.assetId||item.assetId===filters.assetId)&&(!filters.urlId||item.urlId===filters.urlId)&&(!filters.endpointId||item.endpointId===filters.endpointId)).map(mockFindingDto)),
  getScannerFindingDetail:async(id:string):Promise<ScannerFindingDetailDto>=>{if(API_MODE==="http")return request(`/scanner-findings/${id}`);const finding=(await coreApi.getScannerFindings()).find(item=>item.id===id);if(!finding)throw new ApiError(404,"NOT_FOUND","Scanner finding not found");return {finding,asset:(await coreApi.getAssets()).find(item=>item.id===finding.assetId)??null,url:finding.urlId?(await coreApi.getUrls()).find(item=>item.id===finding.urlId)??null:null,endpoint:finding.endpointId?(await coreApi.getEndpoints()).find(item=>item.id===finding.endpointId)??null:null,changes:[],relatedScoreEvents:[]};},
  createScannerFinding:async(input:CreateScannerFindingInput):Promise<ScannerFindingDto>=>{if(API_MODE==="http")return request("/scanner-findings",{method:"POST",body:JSON.stringify(input)});throw new ApiError(501,"NOT_IMPLEMENTED","Mock finding creation is not implemented");},
  updateScannerFindingStatus:async(id:string,status:CoreScannerFindingStatus):Promise<ScannerFindingDto>=>{if(API_MODE==="http")return request(`/scanner-findings/${id}/status`,{method:"PATCH",body:JSON.stringify({status})});await api.updateFindingStatus(id,status as never);const item=(await coreApi.getScannerFindings()).find(value=>value.id===id);if(!item)throw new ApiError(404,"NOT_FOUND","Scanner finding not found");return item;},
  ...
  getWorkspaceSummary:async(entityType:WorkspaceEntityType,entityId:string):Promise<EntityWorkspaceSummaryDto>=>API_MODE==="http"?request(`/workspace/${entityType}/${entityId}/summary`):({entityType,entityId,programId:"mock",status:null,notesCount:0,checklist:{total:0,done:0,todo:0,inProgress:0,skipped:0},evidenceCount:0,interestingRequestsCount:0,latestStatusTransition:null}),
  getWorkspaceNotes:async(entityType:WorkspaceEntityType,entityId:string):Promise<ResearchNoteDto[]>=>API_MODE==="http"?request(`/workspace/${entityType}/${entityId}/notes`):[],
  createWorkspaceNote:async(entityType:WorkspaceEntityType,entityId:string,payload:CreateResearchNoteRequest):Promise<ResearchNoteDto>=>request(`/workspace/${entityType}/${entityId}/notes`,{method:"POST",body:JSON.stringify(payload)}),
  updateWorkspaceNote:async(noteId:string,payload:UpdateResearchNoteRequest):Promise<ResearchNoteDto>=>request(`/workspace/notes/${noteId}`,{method:"PATCH",body:JSON.stringify(payload)}),
  deleteWorkspaceNote:async(noteId:string):Promise<void>=>request(`/workspace/notes/${noteId}`,{method:"DELETE"}),
  getWorkspaceChecklists:async(entityType:WorkspaceEntityType,entityId:string):Promise<ManualChecklistDto[]>=>API_MODE==="http"?request(`/workspace/${entityType}/${entityId}/checklists`):[],
  generateWorkspaceChecklist:async(entityType:WorkspaceEntityType,entityId:string):Promise<ManualChecklistDto>=>request(`/workspace/${entityType}/${entityId}/checklists/generate`,{method:"POST"}),
  createWorkspaceChecklist:async(entityType:WorkspaceEntityType,entityId:string,payload:CreateChecklistRequest):Promise<ManualChecklistDto>=>request(`/workspace/${entityType}/${entityId}/checklists`,{method:"POST",body:JSON.stringify(payload)}),
  getChecklistItems:async(checklistId:string):Promise<ChecklistItemDto[]>=>API_MODE==="http"?request(`/workspace/checklists/${checklistId}/items`):[],
  createChecklistItem:async(checklistId:string,payload:CreateChecklistItemRequest):Promise<ChecklistItemDto>=>request(`/workspace/checklists/${checklistId}/items`,{method:"POST",body:JSON.stringify(payload)}),
  updateChecklistItem:async(itemId:string,payload:UpdateChecklistItemRequest):Promise<ChecklistItemDto>=>request(`/workspace/checklist-items/${itemId}`,{method:"PATCH",body:JSON.stringify(payload)}),
  deleteChecklistItem:async(itemId:string):Promise<void>=>request(`/workspace/checklist-items/${itemId}`,{method:"DELETE"}),
  getInterestingRequests:async(entityType:WorkspaceEntityType,entityId:string):Promise<InterestingRequestDto[]>=>API_MODE==="http"?request(`/workspace/${entityType}/${entityId}/interesting-requests`):[],
  createInterestingRequest:async(entityType:WorkspaceEntityType,entityId:string,payload:CreateInterestingRequestRequest):Promise<InterestingRequestDto>=>request(`/workspace/${entityType}/${entityId}/interesting-requests`,{method:"POST",body:JSON.stringify(payload)}),
  updateInterestingRequest:async(requestId:string,payload:Partial<CreateInterestingRequestRequest>):Promise<InterestingRequestDto>=>request(`/workspace/interesting-requests/${requestId}`,{method:"PATCH",body:JSON.stringify(payload)}),
  deleteInterestingRequest:async(requestId:string):Promise<void>=>request(`/workspace/interesting-requests/${requestId}`,{method:"DELETE"}),
  getEvidenceItems:async(entityType:WorkspaceEntityType,entityId:string):Promise<EvidenceItemDto[]>=>API_MODE==="http"?request(`/workspace/${entityType}/${entityId}/evidence`):[],
  createEvidenceItem:async(entityType:WorkspaceEntityType,entityId:string,payload:CreateEvidenceItemRequest):Promise<EvidenceItemDto>=>request(`/workspace/${entityType}/${entityId}/evidence`,{method:"POST",body:JSON.stringify(payload)}),
  updateEvidenceItem:async(evidenceId:string,payload:Partial<CreateEvidenceItemRequest>):Promise<EvidenceItemDto>=>request(`/workspace/evidence/${evidenceId}`,{method:"PATCH",body:JSON.stringify(payload)}),
  deleteEvidenceItem:async(evidenceId:string):Promise<void>=>request(`/workspace/evidence/${evidenceId}`,{method:"DELETE"}),
  updateWorkspaceStatus:async(entityType:WorkspaceEntityType,entityId:string,status:string,reason?:string):Promise<{entityType:WorkspaceEntityType;entityId:string;oldStatus:string|null;status:string}>=>request(`/workspace/${entityType}/${entityId}/status`,{method:"PATCH",body:JSON.stringify({status,reason})}),
