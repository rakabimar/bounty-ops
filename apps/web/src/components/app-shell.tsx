...
const groups = [
  {
    ...
    items: [
      ...
      ["/programs", "Programs", ShieldCheck],
      ["/scope", "Scope manager", Network],
      ["/rules", "Rules of engagement", BookOpen],
    ],
  },
  {
    label: "RECON",
    items: [
      ["/jobs", "Recon jobs", Radar],
      ["/assets", "Asset inventory", Boxes],
      ["/live-hosts", "Live hosts", Globe2],
      ["/urls", "URLs / Pages", FileText],
      ["/api-endpoints", "API endpoints", Braces],
      ["/js-files", "JS files & secrets", Code2],
    ],
  },
  {
    label: "TRIAGE",
  ...
  },
  {
    label: "SYSTEM",
    items: [
      ["/scoring", "Scoring rules", SlidersHorizontal],
      ["/notifications", "Notifications", Bell],
      ["/audit-logs", "Audit logs", History],
      ["/tools", "Tool health", Wrench],
    ...
    <Sidebar collapsible="icon">
        ...
        <Link to="/" className="flex min-w-0 items-center gap-3">
          ...
          {state !== "collapsed" && (
            <div className="min-w-0">
              ...
              <div className="font-mono text-[8px] tracking-[.2em] text-sidebar-foreground/50">
                RECON CONTROL
              </div>
            </div>
          )}
                ...
                {group.items.map(([url, label, Icon]) => (
                  <SidebarMenuItem key={label}>
                    <SidebarMenuButton
                      ...
                      tooltip={label}
                    >
                      <Link to={url as "/"} className="gap-3">
                        <Icon />
                        <span>{label}</span>
                      </Link>
                    </SidebarMenuButton>
      ...
      <SidebarFooter className="border-t border-sidebar-border p-3">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          {state !== "collapsed" && "Local services configured"}
        </div>
      </SidebarFooter>
    </Sidebar>
  );
...
function ShellContent({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (
      ...
      !programs.some(
        (program) =>
          program.id === selectedProgramId && program.status !== "archived",
      )
    )
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset className="min-w-0">
        <header className="sticky top-0 z-30 grid h-16 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 border-b bg-background/90 px-3 backdrop-blur-xl sm:px-5">
          <SidebarTrigger />
          <div className="flex min-w-0 items-center gap-3">
            ...
            >
              <SelectTrigger className="max-w-64 border-0 bg-secondary shadow-none">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <div className="hidden items-center gap-2 border-l pl-3 lg:flex">
              ...
              <span className="text-xs text-muted-foreground">Core API</span>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="gap-2 px-2">
                <div className="grid size-7 place-items-center rounded-full bg-secondary font-mono text-xs">
                  {initials}
                </div>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuLabel>
                <span className="block">Authenticated operator</span>
              ...
              <DropdownMenuItem disabled={auth.logoutPending} onClick={logout}>
                <LogOut />
                Sign out
              </DropdownMenuItem>
            </DropdownMenuContent>
