...
export function CoreJobsPage() {
  ...
  return <div className="space-y-5">
    ...
    <Sheet open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}><SheetContent className="w-full overflow-y-auto sm:max-w-xl"><SheetHeader><SheetTitle>{selected ? display(selected.type) : "Job"} logs</SheetTitle><SheetDescription>Simulated worker lifecycle for {selected?.id}.</SheetDescription></SheetHeader><pre className="mt-6 overflow-auto whitespace-pre-wrap rounded-md bg-sidebar p-4 font-mono text-xs leading-6 text-sidebar-foreground">{logs.isLoading ? "Loading logs…" : logs.isError ? message(logs.error) : (logs.data?.logs ?? []).map((line) => `${line.timestamp} [${line.level}] ${line.message}`).join("\n") || "No logs yet."}</pre></SheetContent></Sheet>
  </div>;
}
